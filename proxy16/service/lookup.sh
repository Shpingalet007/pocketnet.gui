...
for i in "${!mainHosts[@]}"; do
  ...
  if [ "$savedMainIp" != "$lookedMainIp" ]; then
    ...
    # contents=(jq -r '.combat['"${i}"'].main.ip = "'"${lookedMainIp}"'"' peertube-servers.json)
    # echo -E "${contents}" > peertube-servers.json
  ...
  if [ "$savedMirrorIp" != "$lookedMirrorIp" ] && [ "$savedMirrorIp" != "null" ]; then
    ...
    # contents=$(jq -r '.combat['"${i}"'].mirror.ip = "'"${lookedMirrorIp}"'"' peertube-servers.json)
    # echo -E "${contents}" > peertube-servers.json
